...
serial('calls should have the correct bearer token', async t => {
  const TEST_KEY = 'test_key';
  const spyAgent = createSpyAgent(undefined, TEST_KEY);
  const builtResources = createResourcesRoot(testResources)(spyAgent);
  ...
  const {headers} = spyAgent.getArgs();

  t.deepEqual(headers.Authorization, `Bearer ${TEST_KEY}`);
});
